...
class Solution {
public:
    int ladderLength(string strStart, string strEnd, unordered_set<string> &dict)
    {
        initialize_graph(strStart, strEnd, dict);
        traversal(strStart, strEnd);

        if (nodeInfo_[strEnd].visited)
        {
            return nodeInfo_[strEnd].dist + 1;
        }
        else
        {
            return 0;
...
private:
    struct NodeInfo
    {
        bool visited;
        int dist;
        NodeInfo(): visited(false), dist(numeric_limits<int>::max()) {}
    };
private:
    unordered_map<string, vector<string> > graph_;
    unordered_map<string, NodeInfo> nodeInfo_;
private:
    void initialize_graph(string strStart, string strEnd, unordered_set<string> &dict)
    {
        graph_.insert(make_pair(strStart, vector<string>()));
        graph_.insert(make_pair(strEnd, vector<string>()));

        for (unordered_set<string>::iterator it = dict.begin(); it != dict.end(); ++it)
        {
            graph_.insert(make_pair(*it, vector<string>()));
        }

        nodeInfo_.insert(make_pair(strStart, NodeInfo()));
        nodeInfo_.insert(make_pair(strEnd, NodeInfo()));
        for (unordered_set<string>::iterator it = dict.begin(); it != dict.end(); ++it)
        {
            if (edit_distance(*it, strStart) == 1)
            {
                graph_[strStart].push_back(*it);
                graph_[*it].push_back(strStart);
            }
            if (edit_distance(*it, strEnd) == 1)
            {
                graph_[strEnd].push_back(*it);
                graph_[*it].push_back(strEnd);
            }

            for (unordered_set<string>::iterator it2 = dict.begin(); it2 != dict.end(); ++it2)
            {
                if (edit_distance(*it, *it2) == 1)
                {
                    graph_[*it].push_back(*it2);
                    graph_[*it2].push_back(*it);
                }
            }

            nodeInfo_.insert(make_pair(*it, NodeInfo()));
        }


        // cout << nodeInfo_[strStart].visited << " : " << nodeInfo_[strStart].dist << endl;
        /*
        cout << "size of map: " << graph_.size() << endl;
        vector<string> vec = graph_[strEnd];
    ...
    // computes edit distance of equi length strings
    ...
    void traversal(string strStart, string strEnd)
    {
        queue<string> Q;
        Q.push(strStart);
        nodeInfo_[strStart].visited = true;
        nodeInfo_[strStart].dist = 0;
        ...
        while (!Q.empty())
        {
            ...
            for (vector<string>::iterator it = graph_[curStr].begin(); it != graph_[curStr].end(); ++it)
            {
                string nextStr = *it;
                if (!nodeInfo_[*it].visited)
                {
                    Q.push(*it);
                    nodeInfo_[*it].visited = true;
                    nodeInfo_[*it].dist = nodeInfo_[curStr].dist + 1;
                }
            }
...
int main(int argc, char* argv[])
{
    ...
    Solution sln;
    int len = sln.ladderLength(strStart, strEnd, dict);
    return 0;
}
